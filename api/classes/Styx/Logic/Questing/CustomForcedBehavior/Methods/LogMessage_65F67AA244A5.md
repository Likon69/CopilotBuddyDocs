# CustomForcedBehavior.LogMessage Method

Writes a log message.

## Namespace
[Styx.Logic.Questing](../../../../../../namespaces/Styx/Logic/Questing.md)

## Assembly
CopilotBuddy (in CopilotBuddy.dll)

## Syntax
```csharp
public void LogMessage(string messageType, Color? messageColor, string format, params object[] args)
```

## Parameters

| Name | Type | Description |
| --- | --- | --- |
| messageType | string | The message type. |
| messageColor | Color? | The message color. |
| format | string | The format. |
| args | object[] | The args. |

## See Also
[CustomForcedBehavior Class](../../CustomForcedBehavior.md)
[Styx.Logic.Questing Namespace](../../../../../../namespaces/Styx/Logic/Questing.md)
