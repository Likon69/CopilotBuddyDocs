# WoWSpellFocus.WestriftSeismicAnomaly Field

Represents the westrift seismic anomaly value.

## Namespace
[Styx.Logic.Combat](../../../../../../namespaces/Styx/Logic/Combat.md)

## Assembly
CopilotBuddy (in CopilotBuddy.dll)

## Syntax
```csharp
public const WoWSpellFocus WestriftSeismicAnomaly
```

## Field Value

Type: [WoWSpellFocus](../../WoWSpellFocus.md)
The westrift seismic anomaly value.

## See Also
[WoWSpellFocus Enumeration](../../WoWSpellFocus.md)
[Styx.Logic.Combat Namespace](../../../../../../namespaces/Styx/Logic/Combat.md)
