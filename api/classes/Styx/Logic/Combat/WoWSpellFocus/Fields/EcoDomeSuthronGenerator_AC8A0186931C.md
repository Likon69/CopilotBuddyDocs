# WoWSpellFocus.EcoDomeSuthronGenerator Field

Represents the eco dome suthron generator value.

## Namespace
[Styx.Logic.Combat](../../../../../../namespaces/Styx/Logic/Combat.md)

## Assembly
CopilotBuddy (in CopilotBuddy.dll)

## Syntax
```csharp
public const WoWSpellFocus EcoDomeSuthronGenerator
```

## Field Value

Type: [WoWSpellFocus](../../WoWSpellFocus.md)
The eco dome suthron generator value.

## See Also
[WoWSpellFocus Enumeration](../../WoWSpellFocus.md)
[Styx.Logic.Combat Namespace](../../../../../../namespaces/Styx/Logic/Combat.md)
