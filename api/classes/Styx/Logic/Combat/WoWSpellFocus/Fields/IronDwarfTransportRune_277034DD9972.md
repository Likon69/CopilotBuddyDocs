# WoWSpellFocus.IronDwarfTransportRune Field

Represents the iron dwarf transport rune value.

## Namespace
[Styx.Logic.Combat](../../../../../../namespaces/Styx/Logic/Combat.md)

## Assembly
CopilotBuddy (in CopilotBuddy.dll)

## Syntax
```csharp
public const WoWSpellFocus IronDwarfTransportRune
```

## Field Value

Type: [WoWSpellFocus](../../WoWSpellFocus.md)
The iron dwarf transport rune value.

## See Also
[WoWSpellFocus Enumeration](../../WoWSpellFocus.md)
[Styx.Logic.Combat Namespace](../../../../../../namespaces/Styx/Logic/Combat.md)
